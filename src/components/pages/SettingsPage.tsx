...
type SettingsSubTab = 'general' | 'applications' | 'skills' | 'logs';
...
export default function SettingsPage() {
  ...
  const handleImportConfig = async () => {
    ...
    const initialSteps = [
      ...
      { label: '恢复技能配置', status: 'pending' as const },
  ...
  const [skillsToolConfig, setSkillsToolConfig] = useState<Record<string, { name?: string; skills_path?: string; mode?: string; enabled_skills?: string[] }>>({});
  ...
  const loadSkills = useCallback(async () => {
    ...
    try {
      const [skillList, toolConfig] = await Promise.all([
        listSkills(),
        getSkillsToolConfig() as Promise<Record<string, { name?: string; skills_path?: string; mode?: string; enabled_skills?: string[] }>>,
      ]);
      setSkills(skillList);
  ...
  const handleCollectSkills = async () => {
    ...
    try {
      ...
      const toolConfig = await getSkillsToolConfig() as Record<string, { name?: string; skills_path?: string; mode?: string; enabled_skills?: string[] }>;
      setSkillsToolConfig(toolConfig);
      ...
      <Dialog open={exportDialogOpen} onOpenChange={setExportDialogOpen}>
        <DialogContent className="sm:max-w-[520px]">
          <DialogHeader>
            ...
            <DialogDescription>
              导出时会先选择目标目录，然后在该目录生成 `AAStationConfig.zip`。压缩包内固定包含 `hash.txt`、`metrics.json`、`pipeline.json`、`settings.json`、`skills_config.json`、`skills/` 目录和 `manifest.json`。
            </DialogDescription>
          </DialogHeader>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {skill.hasSkillMd && (
                            <Badge variant="outline" className="text-[10px] px-1.5 py-0">SKILL.md</Badge>
                          )}
                        </div>
                        {skill.description && (
                        ...
                        {toolIds.map((toolId) => {
                          ...
                          return (
                            <button
                              key={toolId}
                              type="button"
                              onClick={() => isEnabled ? handleDisableSkill(skill.name, toolId) : handleEnableSkill(skill.name, toolId)}
                              className={cn(
            ...
            {toolIds.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {toolIds.map((toolId) => {
                  ...
                  const enabledCount = skills.filter((s) => s.enabledInTools.includes(toolId)).length;
                    ...
                    >
                      <div className="p-3.5">
                        <div className="flex justify-between items-center">
                          <button
                            type="button"
                            onClick={() => setExpandedSkillTool((v) => (v === toolId ? null : toolId))}
                            className="bg-transparent border-none text-foreground p-0 m-0 cursor-pointer text-left flex-1"
                          >
                            <div className="flex items-center gap-2.5">
                              <span className="text-sm font-semibold text-foreground">{tool?.name ?? toolId}</span>
                              <Badge variant={enabledCount > 0 ? 'success' : 'outline'} className="text-[10px]">
                                {enabledCount}/{skills.length} 已启用
                              </Badge>
                            </div>
                            <div className="text-[11px] text-dim mt-0.5">{tool?.skills_path ?? ''}</div>
                          </button>
