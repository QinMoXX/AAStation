    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-[520px] max-w-full overflow-y-auto border-l border-border-soft bg-background sm:max-w-[520px]">
        <SheetHeader>
          ...
          <SheetDescription className="text-xs text-muted">{request.id}</SheetDescription>
        </SheetHeader>
        ...
        <div className="px-1 pb-6">
          ...
          <DetailRow label="完整路径">
            <span className="break-all">{request.method} {request.path}</span>
          </DetailRow>
          ...
          <DetailRow label="耗时">{formatDuration(request.duration_ms)}</DetailRow>
          <DetailRow label="时间">
            <div className="leading-relaxed">
          ...
          <DetailRow label="错误信息">
            <span className={cn("whitespace-pre-wrap break-words", request.error ? "text-destructive" : "text-muted")}>
              {request.error || '无'}
            </span>
          </DetailRow>
        </div>
  ...
  return (
    <Card className="relative overflow-hidden border-border bg-card/90 shadow-[var(--color-shadow-soft)]">
      <CardContent className="p-5">
        ...
        <div className="text-[30px] font-bold text-foreground mb-2">{value}</div>
            ...
            {rows.slice(0, 6).map((row) => (
              <div key={row.id}>
                <div className="flex justify-between gap-3 text-[13px] mb-1.5">
                  <span className="text-foreground overflow-hidden text-ellipsis">{row.label}</span>
  ...
  return (
    <Card className="border-border bg-card/90 shadow-[var(--color-shadow-soft)]">
      <CardContent className="p-5">
        ...
        >
          <span className="text-base font-semibold text-foreground">运行时状态</span>
          <ChevronRight
            className={cn(
        ...
        {!collapsed && (
          <div className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] gap-4 mt-4">
            <div className="flex flex-col gap-2.5">
              ...
              {providers.length === 0 && <div className="text-dim text-[13px]">暂无供应商运行态数据</div>}
              {providers.slice(0, 8).map((provider) => (
                ...
                >
                  <div className="flex justify-between gap-3 items-center">
                    <div className="text-foreground text-[13px] font-semibold">{provider.provider_label}</div>
                    <Badge
                      variant="outline"
                  ...
                  <div className="mt-2.5">
                    <div className="text-[11px] text-muted mb-1.5">熔断时间线</div>
                    <div className="flex flex-col gap-1.5">
                      {provider.timeline.length === 0 && (
                        <div className="text-[11px] text-dim">暂无熔断事件</div>
                      )}
                      {provider.timeline.slice(-4).reverse().map((event) => (
                        <div key={`${provider.provider_id}-${event.at}-${event.kind}`} className="text-[11px] text-muted leading-relaxed">
            ...
            <div className="flex flex-col gap-2.5">
              ...
              {pollers.length === 0 && <div className="text-dim text-[13px]">暂无轮询节点运行态数据</div>}
              {pollers.slice(0, 8).map((poller) => (
                <div
                  key={poller.poller_id}
                  className="rounded-xl border border-border bg-surface/55 p-3"
                >
                  <div className="text-foreground text-[13px] font-semibold">{poller.poller_label}</div>
                  <div className="grid grid-cols-2 gap-2 mt-2.5 text-xs text-muted">
                  ...
                  <div className="mt-2.5">
                    ...
                    <div className="flex flex-col gap-1.5">
                      {poller.target_stats.length === 0 && (
                        <div className="text-[11px] text-dim">暂无目标命中数据</div>
                      )}
                      {poller.target_stats.slice(0, 4).map((stat) => (
                        <div
                          key={`${poller.poller_id}-${stat.target_id}`}
                          className="text-[11px] text-foreground grid gap-2"
                          style={{ gridTemplateColumns: showTargetWeight(poller.strategy) ? '1fr auto auto' : '1fr auto' }}
                        >
                          <span>{stat.target_label || stat.target_id}</span>
                          {showTargetWeight(poller.strategy) && (
                            <span className="text-purple-300">权重 {stat.configured_weight}</span>
                          )}
                          <span className="text-muted">命中 {formatNumber(stat.hits)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
...
export default function MonitorPage() {
  ...
  const handleExportRequests = useCallback(() => {
    exportCsv(
      ...
      [
        '请求ID', '完成时间', '应用', '端口', '供应商', '方法', '路径', '协议',
        '请求模型', '路由模型', '响应模型', '状态码', '成功', '流式',
        '输入Token', '输出Token', '总Token', '耗时ms', '错误信息',
      ],
      filteredRequests.map((item) => [
        item.id, item.completed_at, item.app_label, item.listen_port,
        item.provider_label, item.method, item.path, item.protocol,
        item.request_model, item.target_model, item.response_model,
        item.status_code, item.success, item.streamed,
        item.input_tokens, item.output_tokens, item.total_tokens,
        item.duration_ms, item.error,
      ])
    );
  }, [filteredRequests, timeRange]);
    ...
    <div className="ui-page ui-accent-monitor flex flex-1 overflow-auto">
      ...
      >
        ...
        <Card className="border-border bg-card/92 shadow-[var(--color-shadow-soft)]">
          <CardContent className="flex flex-wrap justify-between gap-6 p-6">
            <div className="min-w-[280px]">
              <div className="mb-2 text-[28px] font-bold text-foreground">API 监控中心</div>
            ...
            <div className="grid grid-cols-2 gap-3 min-w-[320px] flex-1" style={{ gridTemplateColumns: 'repeat(2, minmax(180px, 1fr))' }}>
              <Card className="rounded-2xl border-border bg-surface/70 shadow-none">
                <CardContent className="p-4">
                  ...
                  <div className={cn("text-xs mt-2", proxyStatus.running ? "text-green-300" : "text-dim")}>
                    {proxyStatus.running
        ...
        <Card className="border-border bg-card/92 shadow-[var(--color-shadow-soft)]">
          <CardContent className="flex flex-wrap items-end gap-3.5 p-[18px]">
            ...
            <div className="min-w-[220px]">
              <Label className="text-xs text-muted mb-2 block">供应商筛选</Label>
            ...
            {error && (
              <div className="text-xs text-destructive">数据拉取失败: {error}</div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-border bg-card/92 shadow-[var(--color-shadow-soft)]">
          <CardContent className="p-5">
            <div className="flex justify-between items-center gap-3 flex-wrap mb-4">
              <div>
                <div className="text-base font-semibold text-foreground">最近请求明细</div>
              ...
              <div className="flex gap-2 items-center flex-wrap">
                ...
                <Button variant="secondary" size="xs" onClick={handleExportTrend} className="gap-1.5">
                  <Download className="w-3 h-3" /> 导出趋势 CSV
                </Button>
                <Button variant="accent" size="xs" onClick={handleExportRequests} className="gap-1.5">
            ...
            {loading ? (
              ...
              <div className="text-dim text-[13px]">暂无符合当前筛选条件的请求数据</div>
            ) : (
                    ...
                    {filteredRequests.slice(0, 18).map((item) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-3">
                          ...
                          <div className="text-[11px] text-dim mt-0.5">:{item.listen_port}</div>
                        </td>
                        <td className="px-3 py-3">{item.provider_label}</td>
                        <td className="px-3 py-3">
                          <div>{item.method} {item.path}</div>
                          <div className="text-[11px] text-dim mt-0.5">{item.protocol}</div>
                        </td>
                        <td className="px-3 py-3">
                        ...
                        <td className="px-3 py-3">
                          ...
                          {item.error && (
                            <div className="text-[11px] text-destructive mt-1.5">{item.error}</div>
                          )}
                        </td>
                      </tr>
                    ))}
