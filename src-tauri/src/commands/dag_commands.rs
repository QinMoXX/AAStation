...
pub async fn publish_dag(
    ...
    mut doc: DAGDocument,
) -> Result<crate::proxy::types::RouteTableSet, String> {
    // 1. Validate
    ...
    // 2. Load settings for listen address/port range
    let app_settings = settings::load_settings().map_err(|e| e.to_string())?;

    // 3. Auto-assign ports to Application nodes that don't have one
    ...
    let mut assigned_ports: Vec<u16> = used_ports.clone();

    for node in &mut doc.nodes {
        if node.node_type != crate::dag::types::NodeType::Application {
            continue;
        }
        if let Ok(mut data) = serde_json::from_value::<crate::dag::types::ApplicationNodeData>(node.data.clone()) {
            if data.listen_port == 0 {
                ...
                if next_port > port_end {
                    return Err(format!("No available port in range {} (all ports in use)", app_settings.listen_port_range));
                }
    ...
    for node in &mut doc.nodes {
        ...
        if let Ok(mut data) = serde_json::from_value::<crate::dag::types::ApplicationNodeData>(node.data.clone()) {
            if data.listen_port == 0 {
                ...
                if next_port > port_end {
                    return Err(format!("No available port in range {}", app_settings.listen_port_range));
                }
                data.listen_port = next_port;
                assigned_ports.push(next_port);
                next_port += 1;
